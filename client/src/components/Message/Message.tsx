import { useContext, useEffect, useMemo, ComponentProps } from "react";
...
type Props = ComponentProps<"div"> & MessageI & {
    prevMessage: MessageI,
    chatUser?: User,
    relationship?: Relationship,
    nextMessage: MessageI
}
...
export default function Message(props: Props) {
    ...
    const handleRemove = async () => {
        await updateMessage(token, props._id, props.chat_id, null, true)
    }
    ...
    useEffect(() => {
        if(!user || typeof props.user_id === 'string') return
    ...
    }, [inView, props._id, props.chat_id, props.isRead, props.user_id, token, user!._id])

    if(!user || typeof props.user_id === 'string'){
        return <></>
    }
            ...
            <div className={user._id === props.user_id._id ? "unsent-message message mine " + messageState : "unsent-message " + messageState}>
                ...
                <div className="message-content" ref={ref}>
                    ...
                    <div className="message-content-message">{user._id === props.user_id._id ?
                        ...
                        props.chatUser && props.chatUser.first_name + " unsent a message"}</div>
                    {user._id !== props.user_id._id &&
                    ...
                    <div className="message-state">
                        ...
                        <img id="read" src={props.chatUser && props.chatUser.profile} alt="User message" />
        ...
        return (
            <div className='first-message'>
                <div className="first-message-user">
                    <img src={props.chatUser && props.chatUser.profile} alt="Chat user" />
                </div>
                <div className="first-message-username">{props.chatUser && props.chatUser.first_name} {props.chatUser && props.chatUser.last_name}</div>
                <div className="first-message-data">
                    <div>Odinbook</div>
                    <div>
                        {props.relationship && props.relationship._id && props.relationship.request_state === 'Accepted' ? "You're" : "You're not"} friends on Odinbook</div>
                </div>
            </div>
        )
    }
