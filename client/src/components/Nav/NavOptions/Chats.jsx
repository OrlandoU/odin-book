...
export default function Chats() {
    ...
    const { openChats, addChat } = useContext(ChatContext)
    ...
    const handleQueriedUser = async (user) => {
        const chat = await getChatWithUser(token, user._id)
        if (!chat) {
            const newChat = await createChat(token, user._id)
            setContent('')
            addChat(newChat)
        } else {
            setContent('')
