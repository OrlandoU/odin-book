...
import { getChats, getUnviewedChats, updateMessagesToViewed } from "../../../functions/chat";
...
export default function Chats() {
    ...
    const handleOpenMessages = () => {
        unviewedChats.forEach(async chat => await updateMessagesToViewed(token, chat._id))
        setUnviewedChats([])
    }

    const handleSocket = useCallback(()=>{
        const newMessageHandler = (message) => {
            if (message.user_id._id !== user._id && !openChats.find(chat => (message.chat_id === chat._id && chat.open))) {
