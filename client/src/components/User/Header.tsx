    ...
    const handleRemoveRequest = () => {
        if (url.userId) {
            removeFriend(token, url.userId)
                .then(req => req && setRelationship(req))
        }
    }
    ...
    return (
        <header className="user-header">
            <div className="header-wrapper">
                <div className="cover">
                    {props.user.cover && <img src={props.user.cover} alt="Profile cover" />}
                </div>
                <div className="user-info">
                    <div className="user-profile-pic">
                        <div className="user-profile-pic-wrapper">
                            {props.user.profile && <img src={props.user.profile} alt="" />}
                        </div>
                    </div>
                    <div className="user-details">
                        <div className="user-name">{props.user.first_name} {props.user.last_name}</div>
                        <div className="user-friends-count">{props.friends.length} {props.friends.length !== 1 ? 'Friends' : 'Friend'}</div>
                ...
                {(relationship && relationship.request_state === 'Pending' && relationship.sender_id !== user._id) &&
                    <div className="user-relationship-confirm-wrapper">
                        <div className="user-relationship-confirm">
                            <div className="sub-title-smaller">{props.user.first_name} sent you a friend request</div>
                            <div className="user-options">
