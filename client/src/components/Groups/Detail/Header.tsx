import { useContext, useEffect, useMemo, useState, ChangeEventHandler, MouseEventHandler, KeyboardEventHandler } from "react"
...
export default function Header(props: Props): JSX.Element {
    ...
    const handleSave: MouseEventHandler = (e) => {
        if (image) {
            updateGroup(token, props._id, props.name, image)
        }
    }
    ...
    const handleContent: KeyboardEventHandler<HTMLInputElement> = (e) => {
        const target = e.target as HTMLInputElement
        setContent(target.value)
        if (target.value.length > 0) {
            queryUser(token, target.value)
                .then(value => {
                    value && setUsers(value.filter(user => !user.groups.includes(props._id)))
                })
        } else {
            setUsers([])
        }
    ...
    const handleJoinGroup: MouseEventHandler = async () => {
        setInvitation([])
        await joinGroup(token, props._id)
        if (updateUser) updateUser(token)
        await deleteMultipleNotification(token, { group: props._id, user_id: user!._id, type: 'invite' })
    }

    const handleDeclineInvite: MouseEventHandler = async () => {
        await deleteMultipleNotification(token, { group: props._id, user_id: user!._id, type: 'invite' })
        setInvitation([])
    }

    const isMember: boolean | null = useMemo(() => {
        if (!user!._id) return null
        return user!.groups.find(group => typeof group !== 'string' && group._id === props._id) ? true : false
    ...
    useEffect(() => {
        ...
        getFriends(token, user!._id, { limit: 25 }).then(res => {
            Array.isArray(res) && setFriends(res.map(rel => rel.user1_id._id !== user._id ? rel.user1_id : rel.user2_id))
        })
        ...
        <header className="user-header">
            <div className="header-wrapper">
                ...
                <div className="cover">
                    ...
                    <label htmlFor="cover-input" className="user-edit-option user-option"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><title>pencil</title><path d="M20.71,7.04C21.1,6.65 21.1,6 20.71,5.63L18.37,3.29C18,2.9 17.35,2.9 16.96,3.29L15.12,5.12L18.87,8.87M3,17.25V21H6.75L17.81,9.93L14.06,6.18L3,17.25Z"></path></svg>Edit</label>
                </div>
                ...
                {invitation.length > 0 &&
                    <div className="user-relationship-confirm-wrapper">
                        <div className="user-relationship-confirm">
                            <div className="sub-title-smaller">{typeof invitation[0].sender_id !== 'string' && invitation[0].sender_id!.first_name} invited you</div>
